...
import { Calendar, ArrowDown, ArrowLeft } from 'lucide-react';
...
export function CamaraRefrigeradaHistoryDialog({ historico, loading }: CamaraRefrigeradaHistoryDialogProps) {
    ...
    <DialogContent className="max-w-2xl">
      <DialogHeader>
        <DialogTitle className="flex items-center gap-2">
          ...
          Histórico da Câmara Refrigerada
        </DialogTitle>
        <DialogDescription>
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <Badge 
                      variant={item.tipo === 'retirada' ? 'destructive' : 'default'}
                      className="text-xs"
                    >
                      {item.tipo === 'retirada' ? 'Retirado' : 'Retorno ao Freezer'}
                    </Badge>
                  </div>
