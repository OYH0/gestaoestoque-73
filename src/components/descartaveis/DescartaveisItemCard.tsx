...
import { Minus, Plus, Check, X, Package, Trash2 } from 'lucide-react';
...
export function DescartaveisItemCard({
  item,
  isEditing,
  editValue,
  onStartEdit,
  ...
  onCancelEdit,
  onDelete
}: DescartaveisItemCardProps) {
  const isLowStock = item.quantidade <= (item.minimo || 10);
      ...
      <CardContent className="p-3 sm:p-4">
        <div className="flex items-center justify-between">
          <div className="flex-1">
            <div className="flex items-center gap-2 sm:gap-3 mb-2">
              <div className="flex items-center gap-2">
                <Package className="w-3 h-3 sm:w-4 sm:h-4 text-green-500" />
          ...
          <div className="flex flex-col gap-2 ml-2 sm:ml-4">
            {isEditing ? (
              <div className="flex items-center gap-1 sm:gap-2 bg-green-50 p-2 rounded-lg">
                <Button
                  size="sm"
                  variant="outline"
                ...
                <Button
                  ...
                  className="bg-green-500 hover:bg-green-600 h-6 w-6 sm:h-8 sm:w-8 p-0"
                >
